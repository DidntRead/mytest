...
/**
 * @file   drApiMarshal.h
 * @brief  Contains marshaling definitions and data structures
 *
 */
 ...
 */
typedef struct {
    //reqPayloadHeader_t header;
    uint32_t functionId;
    union {
        //drMarshalingData_t  sampleData;  /** Data */
        tlApiResult_t retVal;
        struct tlfocalApiDevice *devData;
    } payload;
    uint32_t     sid;           /** Session identifier */
} drMarshalingParamTemplate_t, *drMarshalingParamTemplate_ptr;
