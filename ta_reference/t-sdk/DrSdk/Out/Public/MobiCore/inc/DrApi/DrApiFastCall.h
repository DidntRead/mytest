...
/* For AArch32, the 0x82XXXXXX are trapped by the EL3 and only the 0x81XXXXXX
   SMC are forwarded to <t-base */
#define FASTCALL_OWNER_SIP     (0x81000000)
#else
#define FASTCALL_OWNER_SIP_OLD (0x81000000) /* Do not use, only for backward compatibility */
#define FASTCALL_OWNER_SIP     (0x82000000)
#endif
...
#define FASTCALL_FLAG_SWD   (1 << 0)

#define FCH_L1_MAX  12

#if defined(__AARCH32__)
typedef uint64_t fastcall_registers_t;
#else
 ...
 *   flags contains input fastcall flags(FASTCALL_FLAG_SWD for SWD source of fastcall)
 * On exit
