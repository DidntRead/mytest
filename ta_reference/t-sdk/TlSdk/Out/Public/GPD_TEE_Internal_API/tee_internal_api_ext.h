...
//------------------------------------------------------------------------------
_TLAPI_EXTERN_C TEE_Result TEE_TBase_UnwrapObject(
        void                *src,
        size_t              srcLen,
        void                *dest,
        size_t              destLen);
...
//------------------------------------------------------------------------------
_TLAPI_EXTERN_C TEE_Result TEE_TBase_DeriveKey(
        const void          *salt,
        size_t              saltLen,
        void                *dest,
        size_t              destLen);
 ...
 */
_TLAPI_EXTERN_C TEE_Result TEE_TBase_TUI_GetScreenInfo(
 ...
 * @return  TLAPI_OK upon success or specific error
 */
_TLAPI_EXTERN_C TEE_Result TEE_TBase_TUI_OpenSession(void);
 ...
 * @return  TLAPI_OK upon success or specific error
 */
_TLAPI_EXTERN_C TEE_Result TEE_TBase_TUI_CloseSession(void);
 ...
 * @return  TLAPI_OK upon success or specific error.
 */
_TLAPI_EXTERN_C TEE_Result TEE_TBase_TUI_SetImage(
 ...
 * @return  TLAPI_OK upon success or specific error.
 */
_TLAPI_EXTERN_C TEE_Result TEE_TBase_TUI_GetTouchEvent(
 ...
 * @param  output       [in]  reference to the output address of the decrypted data buffer, can be address/ID/handle or other
 *
...
/**
 * Create a new heap.
 * The address of the allocated heap is aligned on a 8-bytes boundary.
 *
 * @param   heapStartSize: [in] the number of bytes to be allocated at heap init.
 *
 * @param   heapMaxSize: [in] the maximum number of bytes of the heap.
 *
 * @param   flags: [in] RFU
 *
 * @param   hint: [out] id of the heap
 *
 * @return  TLAPI_OK upon success or specific error.
 **/

_TLAPI_EXTERN_C TEE_Result TEE_TBase_CreateHeap(
    uint32_t heapStartSize,
    uint32_t heapMaxSize,
    uint32_t flags,
    uint32_t *hint);

#endif /* __TLAPIHEAP_H__ */
 ...
 *
 * @param [in] msg to endorse (i.e. create endorsement of)
 * @param [in] msgLen Length of message to endorse
 * @param [out] dst the endorsement. If dst is NULL destLen is updated
 *              with the required length of the buffer to store the endorsement
 * @param [in, out] dstLen Length of the endorsement
 * @retval TEE_SUCCESS if OK
 * @retval TEE_ERROR_BAD_PARAMETERS dst and dstLen are both NULL or msgLen is
 *                                  too large.
 * @retval TEE_ERROR_SHORT_BUFFER if dst is non-NULL and dstLen is too small
 * @retval TEE_ERROR_GENERIC any other error condition
 */

TEE_Result TEE_TBase_Endorse(
    const void *msg,
    size_t      msgLen,
    void       *dst,
    size_t     *dstLen);
 ...
 * Reseed DRBG
 *
 * @param  buf          [in]  Buffer containing user-supplied entropy
 * @param  buflen       [in]  Length of buffer
 *
 * @return TEE_SUCCESS upon success
 *
 */
_TLAPI_EXTERN_C TEE_Result TEE_TBase_AddEntropy(
        uint8_t* buf,
        uint32_t buflen);
