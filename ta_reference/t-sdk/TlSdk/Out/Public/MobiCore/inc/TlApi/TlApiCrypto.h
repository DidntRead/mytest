...
#if TBASE_API_LEVEL >= 4
#define AF_SIG_AES      (8U << 16)
#define AF_SIG_DSA      (16U << 16)
#define AF_SIG_ECDSA    (32U << 16)
#endif
...
#if TBASE_API_LEVEL >= 4
    /*------- DSA starts here -------*/
    TLAPI_SIG_DSA_RAW                       = (AF_SIG | AF_SIG_DSA | 1),    /**< DSA with raw data. No digest calculation. */

    /*------- ECDSA starts here -------*/
    TLAPI_SIG_ECDSA_RAW                     = (AF_SIG | AF_SIG_ECDSA | 1),  /**< ECDSA with raw data. No digest calculation. */
...
typedef enum {
    ...
    TLAPI_ALG_PLATFORM_RANDOM = (AF_RNG | 3),  /**< Raw data from the platform's TRNG (not cryptographically secure) */
#endif
} tlApiRngAlg_t;
...
#if TBASE_API_LEVEL >= 4
    TLAPI_DSA       = 0x00000002,  /**< DSA public / private key. */
    TLAPI_ECDSA     = 0x00000003   /**< ECDSA public / private key. */
#endif
} tlApiKeyPairType_t;
...
/** DSA key structure.
 *
 * Key structure may contain parameters, public key, both public key and private key or
 * Private key only.
 *
 *
 */
typedef struct {
    tlApiLongInt_t  p;    /**< Prime modulus p */

    tlApiLongInt_t  q;    /**< Prime q */

    tlApiLongInt_t  g;    /**< Generator */

    tlApiLongInt_t  x;    /**< Private key */

    tlApiLongInt_t  y;    /**< Public key */
} tlApiDsaKey_t;


/** ECDSA key structure.
 *
 * Key structure may contain curve type, public key, both public key and private key or
 * Private key only.
 *
 * In all cases curve type must be present.
 *
 */
typedef struct {
    uint32_t       curveType;   /**< Curve type (e.g. P192, P224, P256, P384 and P521) */

    tlApiLongInt_t  x;          /**< Public key affine point x */

    tlApiLongInt_t  y;          /**< Public key affine point y */

    tlApiLongInt_t  privKey;    /**< Private key data */
} tlApiEcdsaKey_t;
#endif
...
#if TBASE_API_LEVEL >= 4
    tlApiDsaKey_t   *dsaKey;            /**< Pointer to DSA key. */
    tlApiEcdsaKey_t *ecdsaKey;          /**< Pointer to ECDSA key. */
#endif
} tlApiKey_t;
...
#if TBASE_API_LEVEL >= 4
    tlApiDsaKey_t   *dsaKeyPair;       /**< Pointer to DSA key structure. */
    tlApiEcdsaKey_t *ecdsaKeyPair;     /**< Pointer to ECDSA key structure. */
#endif
} tlApiKeyPair_t;
...
#if TBASE_API_LEVEL >= 5
/** Reseeds the DRBG with supplied entropy.
 *
 * @param buf    Buffer containing supplied entropy.
