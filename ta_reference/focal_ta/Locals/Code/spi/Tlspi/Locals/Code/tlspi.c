...
_TLAPI_ENTRY void tlMain( const addr_t tciBuffer, const uint32_t tciBufferLen )
{
    ...
    tciSpiMessage_t* message = (tciSpiMessage_t*) tciBuffer;
		...
		transfer_data.tx_buf = message->tx_buf ;
		transfer_data.rx_buf = message->rx_buf ;
		transfer_data.len = message->len ;
		transfer_data.chip_config= message->tl_chip_config ;
		transfer_data.is_transfer_end = message->is_transfer_end ;
		transfer_data.cs_change = 0 ; //message->cs_change;
	tlDbgPrintLnf("SEC SPI transfer_data cs_change = %d ---------\n" ,transfer_data.cs_change );
	#endif
