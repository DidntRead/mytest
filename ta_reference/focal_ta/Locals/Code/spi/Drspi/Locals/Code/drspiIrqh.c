...
void SpiHandleIRQ(void)
{
	...
	if(is_last_xfer == 1 && xfer->is_transfer_end == 1)
	{
		SetPauseStatus(IDLE);
	     ...
	     if(xfer->cs_change) { //select non-default device cs pin
	      //set ASPI GPIO mode and out and high
